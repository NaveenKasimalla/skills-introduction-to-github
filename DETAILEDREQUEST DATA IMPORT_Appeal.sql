...
--select  * from DBOps.dbo.DR_CL1;


--select  * from DBOps.dbo.DR_CL1;
--select  * from DBOps.dbo.DR_CL1;

--select  * from  DBOps.DBO.App_Cal 
...
insert into  FileBatch (InputId, FileName, InProcess, ReceivedDate, CompleteDate, ClientId, BatchAckDirectoryFolder, IsBulkLoad)
			...
			--values 	('2029', 'DR_CL1', 0 ,	getdate(),	NULL	,'545',	NULL, 1 ) --FOR UAT
			values 	('21', 'DR_CL1'	,  0 ,	getdate(),	NULL	,'545',	NULL, 1 ) --FOR PROD
...
select @filebatchid= max(filebatchid) from FileBatch where ClientId = '545' AND filename like '%DR_CL1%'
--select @filebatchid

select distinct
	null as ReviewId,
	AppealID as ExternalReviewId, ---- Data from Ahshay
	null as Id,
 ...
 null as DosEnd,
	...
	--case    when  LTRIM(RTRIM(outcome3)) = 'Denied' then    '1000000044' 
	--		WHEN  LTRIM(RTRIM(outcome3)) = 'Approved' then '18' 
	--	--When  LTRIM(RTRIM(tr_decision)) = 'Duplicate precert' then '1000000042'
	--	WHEN  LTRIM(RTRIM(outcome3)) = 'Modified' THEN  '117' end as DeterminationId,
...
case 
When LTRIM(RTRIM(AppealReason1)) = 'TX denial' THEN 	'1000000046'
When LTRIM(RTRIM(AppealReason1)) = 'Testing Denial'	THEN  '1000000046'
When LTRIM(RTRIM(AppealReason1)) = 'DME Denial'	THEN '1000000046'
When LTRIM(RTRIM(AppealReason1)) = 'MMI' then  '1000000047' 
When LTRIM(RTRIM(AppealReason1)) = 'Surgery Denial' THEN	'1000000046'
When LTRIM(RTRIM(AppealReason1)) = 'Modification of CPT / TX / Frequency' THEN  '117' 
When LTRIM(RTRIM(AppealReason1)) = 'Other'	THEN '1000000046' end as DeterminationId,
  ...
  --null as DeterminationId,
	...
	--1407 as FileBatchId, 
	TxPlan1 as DescriptionText,-- Data from Ahshay
	null as BodyPartId,
...
from  DBOps.dbo.DR_CL1;

--drop table DBOps.dbo.DR_CL1;
...
and ExternalReviewId in (SELECT ExternalReviewId FROM DETAILEDREQUESTLOG (NOLOCK) WHERE FILEBATCHID=84271 AND ERRORMESSAGE !='OK' )
...
where A.ExternalReviewId in (SELECT ExternalReviewId FROM DETAILEDREQUESTLOG (NOLOCK) WHERE FILEBATCHID=84271 AND ERRORMESSAGE !='OK' ) --12
