...
--select  * from DBOps.dbo.DR_Cal where tr_decision like '%Pending%'

--select * from DBOps.dbo.Precert_Detail_MIC

Select * from DBOps.dbo.DR_Cal


--select  * from DBOps.dbo.DR_Cal;
--drop table DBOps.dbo.DR_Cal;

--select * from DetailedRequest where clientid = 545

--alter table DetailedRequest_cal rename to DR_Cal;

--exec sp_rename  'DBops.DBO.DetailedRequest_cal' , 'DBOps.DBO.DR_Cal';
...
insert into  FileBatch (InputId, FileName, InProcess, ReceivedDate, CompleteDate, ClientId, BatchAckDirectoryFolder, IsBulkLoad)
			...
			--values 	('2029', 'DetailedRequest_cal', 0 ,	getdate(),	NULL	,'545',	NULL, 1 ) --FOR UAT
			values 	('21', 'DR_Cal'	,  0 ,	getdate(),	NULL	,'545',	NULL, 1 ) --FOR PROD
...
select @filebatchid= max(filebatchid) from FileBatch where ClientId = '545' AND filename like '%DR_Cal%'
--select @filebatchid

select distinct
	null as ReviewId,
	PrecertID as ExternalReviewId, ---- Data from Ahshay
	null as Id,
	PrecertDetailID
 as ExternalDetailedRequestId,
	...
	null as ICD9Id,
	case When LTRIM(RTRIM(tr_decision)) = 'Administrative Denial' then '1000000046'
		When LTRIM(RTRIM(tr_decision)) = 'MMI reached' then '1000000046'
		when  LTRIM(RTRIM(tr_decision)) = 'Denied' then    '1000000046' 
		WHEN  LTRIM(RTRIM(tr_decision)) = 'Approved' then '18' 
		When  LTRIM(RTRIM(tr_decision)) = 'Duplicate precert' then '1000000048'
		WHEN  LTRIM(RTRIM(tr_decision)) = 'Modified' THEN  '117' 
		--WHEN  LTRIM(RTRIM(tr_decision)) = 'Closed per Carrier' THEN  '1000000048'
        WHEN  LTRIM(RTRIM(tr_decision)) =  'Claims Decision' then '1000000048' 
		WHEN  LTRIM(RTRIM(tr_decision)) =  'IME NS - Denied' then '1000000044'
		--WHEN  LTRIM(RTRIM(tr_decision)) =  'IME Recommended' then '18'
		WHEN  LTRIM(RTRIM(tr_decision)) =  'Pending' then '1000000045' end as DeterminationId,
		--WHEN  LTRIM(RTRIM(tr_decision)) =  'PIP Maxed' then '1000000074' end as DeterminationId,
	...
	--1407 as FileBatchId, 
	null as DescriptionText,-- Data from Ahshay
	null as BodyPartId,
	null as CptNdcModifier,
	AuthorizedServiceCodeFrom as AuthorizedCpt,
...
(Select ExternalReviewId from Reviewstat..Review (nolock) where ExternalReviewId is not null and RefLocationID='24262')
--;
--69315
--and ExternalReviewId in (SELECT ExternalReviewId FROM DETAILEDREQUESTLOG (NOLOCK) WHERE FILEBATCHID=84002 AND ERRORMESSAGE !='OK' )
...
where A.ExternalReviewId in (SELECT ExternalReviewId FROM DETAILEDREQUESTLOG (NOLOCK) WHERE FILEBATCHID=84002 AND ERRORMESSAGE !='OK' ) --12
