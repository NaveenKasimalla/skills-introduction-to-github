...
select * from DBOps.DBO.Notes_Pre_Cal
547pc-1571619
...
insert into  FileBatch (InputId, FileName, InProcess, ReceivedDate, CompleteDate, ClientId, BatchAckDirectoryFolder, IsBulkLoad)
		 --values ('2029', 'Notes_Pre_Cal', 0 ,	getdate(),	NULL	,'547',	NULL, 1 ) --FOR UAT
		  values 	('21',	'Notes_Pre_Cal' ,  0 ,	getdate(),	NULL	,'545',	NULL, 1 ) --FOR PROD
			 

---------- STEP 3  -----------------------------------

declare @filebatchid int 
select @filebatchid= max(filebatchid) from FileBatch where ClientId = '545' AND FILENAME LIKE '%Notes_Pre_Cal%'
--select @filebatchid

select 
precertid as ReviewId,
precertid as ExternalReviewId,
