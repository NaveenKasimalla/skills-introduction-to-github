...
select * from DBOps.DBO.Refc_IME

--select * from RSImport..RSIMPORTS_PACKAGE_INPUT where ClientId = 547

--select * from ReviewStat.Dbo.ETL_Location_Mapping where clientID = 547

declare @fileName varchar (100)

set @fileName= 'Refc_IME' 

insert into  FileBatch (InputId, FileName, InProcess, ReceivedDate, CompleteDate, ClientId, BatchAckDirectoryFolder, IsBulkLoad)
				
			--values 	('2029', 'Refc_IME', 0 ,	getdate(),	NULL	,'545',	NULL, 1 ) --FOR UAT
			values 	('21', 'Refc_IME'	,  0 ,	getdate(),	NULL	,'545',	NULL, 1 ) --FOR PROD
...
select @filebatchid= max(filebatchid) from FileBatch where ClientId = '545' AND filename like '%Refc_IME%'
--select @filebatchid
...
'60' as ServiceTypeId, -- Will be supplied by ReviewStat ID values sent by Dennis
...
'0' as ProcessStatus,
LTRIM(RTRIM(IMEID)) as ExternalReviewId ,
...
from DBOps.dbo.Refc_IME;

--================================================================================================================================================
 
 --drop table DBOps.dbo.Refc_IME;
