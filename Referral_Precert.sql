...
select * from DBOps.DBO.NationWide_RF

--select * from RSImport..RSIMPORTS_PACKAGE_INPUT where ClientId = 543

--select * from RSImport..RSIMPORTS_PACKAGE_INPUT where ClientId = 545

--select * from ETL_Location_Mapping where clientID = 549

declare @fileName varchar (100)

set @fileName= 'NationWide_RF' 

insert into  FileBatch (InputId, FileName, InProcess, ReceivedDate, CompleteDate, ClientId, BatchAckDirectoryFolder, IsBulkLoad)
				
			--values 	('2029', 'NationWide_RF', 0 ,	getdate(),	NULL	,'545',	NULL, 1 ) --FOR UAT
			values 	('2027', 'NationWide_RF'	,  0 ,	getdate(),	NULL	,'545',	NULL, 1 ) --FOR PROD
...
select @filebatchid= max(filebatchid) from FileBatch where ClientId = '545' AND filename like '%NationWide_RF%'
--select @filebatchid
...
'126' as ServiceTypeId, -- Will be supplied by ReviewStat ID values sent by Dennis
...
null as ReceivedDate, -- current null, needs to be updated after history migration, currently mapped to start date
APTP_DOR as CaseCreateDate,
null as DocRecDate,
...
'0' as ProcessStatus,
LTRIM(RTRIM(APTP_Id)) as ExternalReviewId ,
...
from DBOps.dbo.NationWide_RF;

--================================================================================================================================================
 
 --drop table DBOps.dbo.NationWide_RF;
